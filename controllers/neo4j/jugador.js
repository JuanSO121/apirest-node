...
const crearJugador = async (req, res) => {
  ...
  if (!/^[a-zA-Z ]+$/.test(nombre)) {
    return res.status(400).send({ error: 'El nombre del jugador solo debe contener letras y espacios.' });
  }
...
const actualizarJugador = async (req, res) => {
  ...
  if (!id) {
    return res.status(400).send({ error: 'El ID del jugador es obligatorio.' });
  }
